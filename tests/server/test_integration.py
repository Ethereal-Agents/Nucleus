...
async def test_supersession_chain():
    ...
    fb = await memory_write(
        content="Fact B", scope="repo", run_id=run_id, supersedes_hint=fa["fact_id"]
    )
    await memory_write(content="Fact C", scope="repo", run_id=run_id, supersedes_hint=fb["fact_id"])
