...
async def test_mcp_lifecycle():
    ...
    # 8. End run
    import json
    valid_summary = json.dumps([{"content": "Completed auth testing", "scope": "testrepo/src/auth"}])
...
def test_begin_run_sets_context_vars():
    from swarm_memory.core.log import current_run_id, current_arm_id
...
async def test_write_returns_write_result():
    ...
    res = await memory_write(
        content="Fact 1", scope="repo/path", run_id=run_id
    )
...
async def test_write_duplicate_returns_duplicate_status():
    run_res = memory_begin_run(repo="testrepo", agent_id="agent1")
    run_id = run_res["run_id"]
    res1 = await memory_write(
        content="Fact Duplicate", scope="repo/path", run_id=run_id
    )
...
async def test_write_with_supersedes_hint():
    ...
    run_id = run_res["run_id"]
    res1 = await memory_write(
        content="Fact Old", scope="repo/path", run_id=run_id
    )
    res2 = await memory_write(
...
async def test_search_fact_type_filter():
    ...
    await memory_write(content="This is an insight", scope="repo", run_id=run_id, fact_type="insight")
...
async def test_invalidate_custom_valid_to():
    run_res = memory_begin_run(repo="testrepo", agent_id="agent1")
    run_id = run_res["run_id"]
    w_res = await memory_write(content="To be invalidated custom valid_to", scope="repo", run_id=run_id)
    fact_id = w_res["fact_id"]
...
async def test_end_run_stores_token_counts():
    ...
    run_id = res["run_id"]
    await memory_end_run(run_id=run_id, summary="[]", input_tokens=100, output_tokens=50, total_cost_usd=0.05)
    
    from swarm_memory.server.mcp_server import db
    row = db.execute("SELECT input_tokens, output_tokens, total_cost_usd FROM runs WHERE id = ?", [run_id]).fetchone()
