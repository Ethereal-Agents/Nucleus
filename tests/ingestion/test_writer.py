...
async def test_write_fact_all_fact_types(db, mock_embedder, mock_detector):
    ...
    for ft in types:
        res = await writer.write_fact(
            content=f"content {ft}", scope="scope", run_id="run_1", fact_type=ft
        )
...
async def test_write_fact_confidence_boundaries(db, mock_embedder, mock_detector):
    # WRT-04
    writer = FactWriter(db, mock_embedder, mock_detector)
    res_min = await writer.write_fact(
        content="c_min", scope="scope", run_id="run_1", confidence=0.0
    )
    res_max = await writer.write_fact(
        content="c_max", scope="scope", run_id="run_1", confidence=1.0
    )
...
async def test_write_fact_content_hash_formula(db, mock_embedder, mock_detector):
    ...
    expected_hash = hashlib.sha256(b"Formula test|scope/test").hexdigest()
...
def test_write_trajectory_embedding_stored(db, mock_embedder, mock_detector):
    ...
    try:
        row = db.execute(
            "SELECT * FROM trajectories_vec WHERE trajectory_id = ?", [saved[0]["fact_id"]]
        ).fetchone()
        assert row is not None
