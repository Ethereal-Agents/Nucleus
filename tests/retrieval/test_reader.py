...
class TestFactReaderBM25:
    ...
    def test_search_exclude_ids(self, bm25_reader):
        # Find some facts first
        all_results = bm25_reader.search("Redis TTL", scope="test-repo/src/auth", top_k=10)
        assert len(all_results) > 0

        first_id = all_results[0].fact.id

        # Search again but exclude the first id
        filtered_results = bm25_reader.search(
            "Redis TTL", scope="test-repo/src/auth", top_k=10, exclude_ids={first_id}
        )

        filtered_ids = [r.fact.id for r in filtered_results]
